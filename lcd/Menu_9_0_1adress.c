...
static unsigned char VIN_Type_flag=0;//区分组的选择和组内选择
static unsigned char VIN_Type_Counter=0;//  0: 数字    1:A-M         2:N-Z

u8 Addr_Code[6];//用于存储地址
		...
		case KeyValueOk:
			...
			if(Vin_Modify_Flag==1)
				...
				if(VIN_SetFlag==7)//设置完成
					...
					lcd_text12(22,19,"按确认完成设置",14,LCD_MODE_SET);
					lcd_update_all();
